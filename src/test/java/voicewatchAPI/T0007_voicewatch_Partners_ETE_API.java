...
public class T0007_voicewatch_Partners_ETE_API {
	

private static Logger log =LogManager.getLogger(T0007_voicewatch_Partners_ETE_API.class.getName());
	...
	body(vw_payLoads.Partner(vw_reuseable.getSaltString())).
	when().post(vw_resources.Patners()).
	then().assertThat().statusCode(200).and().header(vw_header.Content_Type_Option(),vw_header.Content_Type_Option_value()).and().header(vw_header.Content_Type(),vw_header.content_Type_value()).extract().response();
	               ...
	               log.debug("Partner ID: "+js_post.get("id")); 
	               
	                          int Post_id =js_post.get("id");
	                          
	                          log.debug("Partner Name: : " +vw_reuseable.getSaltString()+ " created successfully");
     ...
     /*           
	  ...
	  when().get(vw_resources.Patners_Path()).
	  then().assertThat().statusCode(200).and().header(vw_header.Content_Type_Option(),vw_header.Content_Type_Option_value()).and().header(vw_header.Content_Type(),vw_header.content_Type_value()).extract().response();
				
				                JsonPath js_get=reuseable.rawToJson(resGet);
				               log.debug("Get respose of Generated Partner ID of Voicewatch: "+js_get.get("id"));  
				        ...
				        if (Get_id==Post_id)
				        {
				        	log.debug("Post Partner ID: "+Post_id+ " and Get Partner ID: "+Get_id+ " is equal,So Test Case Pass successfully" );
				        					        	
				        } else if (Get_id!=Post_id) {
				        	
				        	log.debug("Post Partner ID: "+Post_id+" and Get Partner ID: "+Get_id+ " is not equal,So Test Case failed" );
